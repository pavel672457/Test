-- ============================================
-- DELTA FLY GUI v3.0 - ОПТИМИЗИРОВАННЫЙ ДЛЯ ТЕЛЕФОНА
-- Полное кнопочное управление
-- ============================================
...
local CONFIG = {
    ...
    MaxSpeed = 500,
    MinSpeed = 10,
    FlyKey = Enum.KeyCode.F,  -- Клавиша для быстрого включения
    ButtonSize = 60,
    ButtonSpacing = 8,
    Transparency = 0.2,
}
...
local function createGUI()
    ...
    -- Перетаскивание GUI (для мобильных)
...
-- Функция для обработки удержания
local function setupHold(button, onDown, onUp)
    ...
    button.TouchEnded:Connect(stopHold)
...
-- Toggle Fly
gui.ToggleBtn.MouseButton1Click:Connect(toggleFly)
gui.ToggleBtn.TouchTap:Connect(toggleFly)

-- Noclip
gui.NoclipBtn.MouseButton1Click:Connect(toggleNoclip)
gui.NoclipBtn.TouchTap:Connect(toggleNoclip)
